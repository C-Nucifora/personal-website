...
> **Status:** shipped via `lib/intel/` — tier 2: TS family (ts/tsx/js: hover `K`, `gd` + jumplist `Ctrl+o`/`Ctrl+i`, diagnostics, `:symbols`) and the web stack (css: hover/diagnostics/defs/symbols · json: diagnostics/symbols · html: hover/symbols); tier 3: yaml/toml diagnostics + symbols. Python/Rust evaluated and dropped — no maintained browser-capable servers exist (see docs/superpowers/plans/2026-06-12-intel-phase-d-record.md); they degrade silently to tier 1 as designed.
