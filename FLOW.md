...
> **Status:** tier 2 shipped via `lib/intel/` — TS family (ts/tsx/js: hover `K`, `gd` + jumplist `Ctrl+o`/`Ctrl+i`, diagnostics, `:symbols`) and the web stack (css: hover/diagnostics/defs/symbols · json: diagnostics/symbols · html: hover/symbols). Tier-3 adapters follow per the 2026-06-12 spec.
